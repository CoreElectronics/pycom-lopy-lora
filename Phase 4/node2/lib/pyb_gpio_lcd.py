...
from machine import Pin # Fix for import Pin
import time             # Fix for import delay. Requires: time.delay(n) -> time.sleep(n)
...
class GpioLcd(LcdApi):
    ...
    def hal_write_8bits(self, value):
        ...
        if self._4bit:
            ...
            self.d3_pin(value & 0x08)
            self.d2_pin(value & 0x04)
            self.d1_pin(value & 0x02)
            self.d0_pin(value & 0x01)
            self.hal_write_4bits(value >> 4)

    def hal_write_4bits(self, nibble):
        """Writes 4 bits of data to the LCD."""
        self.d7_pin(nibble & 0x08)
        self.d6_pin(nibble & 0x04)
        self.d5_pin(nibble & 0x02)
        self.d4_pin(nibble & 0x01)
